        ...
        for sp,(n,(i,l)) in enumerate(z.items()):
            ...
            for m,(i2,l2) in z.items():
                for ops in vals:
                    ...
                    for d in l:
                        for d2 in l2:
                            w=max(w,y[(d,d2,ttoi(ops),j-1)]+vals[ops])
                        if m in real and m not in ops:
                            w=max(w,y[(d,m,ttoi(tuple(sorted(ops+(m,)))),j-1)]+vals[ops])
                    if n in real and n not in ops:
                        for d2 in l2:
                            w=max(w,y[(n,d2,ttoi(tuple(sorted(ops+(n,)))),j-1)]+vals[ops])
                        if m in real and m not in ops:
                            if n == m:
                                t = (n,)
                            else:
                                t =(n,m)
                            w=max(w,y[(n,m,ttoi(tuple(sorted(ops+t))),j-1)]+vals[ops])
                    y[(n,m,ttoi(ops),j)] = w
    return y["AA","AA",ttoi(()),26]
